...
class Ueditor extends Base
{
    ...
    public function __construct()
    {
        ...
        file_put_contents('D:/sd.txt',input('savePath')."\n",8);
	...
	public function index(){
        ...
        $CONFIG2 = json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents("static/plugins/Ueditor/php/config.json")), true);
        $action = $_GET['action'];
	...
	private function upFile($fieldName){
		...
		if (true !== $result || empty($file)) {
			...
			$info = $file->rule(function ($file) {
				return  md5(mt_rand());
			})->move('upload/'.$this->savePath);
		}

		if($info){
			$data = array(
				'state' => 'SUCCESS',
				'url' => '/upload/'.$this->savePath.$info->getSaveName(),
	...
	*/
    ...
    public function imageUp()
    {
        ...
        if (true !== $result || !$file) {
            ...
            //todo ossconf放进cache,concache读取
            ...
            if ($ossConfig['oss_switch']) {
                ...
                })->move('upload/'.$savePath);
                if ($info) {
                    $state = "SUCCESS";
                } else {
                    $state = "ERROR" . $file->getError();
                }
                $return_url = '/upload/'.$savePath.$info->getSaveName();
            }
